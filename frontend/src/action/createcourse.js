...
async function createcourse(courseid, address, price) {
  ...
  let createcourse = coursescontract.connect(signer);
  //let tx = await coursescontract.setGreeting('Updated greeting');
  let tx = await createcourse.addcourse(courseid, address, price);
  await tx.wait();
}
