...
const Card = (props) => {
  const { title, price, categorydb, desc, _id, rating } = props;
  const [ratingClass, setClasses] = useState([]);
  const courseId = _id;
    ...
    postData(
      ...
      .then((data) => {
        ...
        if (data.error == "Already enrolled") {
          alert(
            "Already enrolled, go to profile to check your enrolled courses"
          );
        }
      })
      .catch((error) => {
